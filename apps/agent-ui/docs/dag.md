...
- **实例参数对每个节点都注入**：`build_input` 先把 `node.config.params`（剔除 `system.*`/`dw.*` UI 旋钮）
  作为 input 初始键，再把上游端口值按边叠上去；同名冲突以上游端口值为准。所以**有上游边的下游节点
  同样能拿到自己的业务参数**（如 Exec-SQL 的 connection/database/sql），不再只是源节点才有参数。
